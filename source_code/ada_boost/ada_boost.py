...
def ad_train_model(train_dataset_path,train_hyperparameters):
    ...
    model_dir = os.path.join(train_hyperparameters.get('model_dir'),'ada_boost',test_case)
    os.makedirs(model_dir, exist_ok = True)
    tree = DecisionTreeClassifier(max_depth=train_hyperparameters.get('max_depth'))
    ...
    for i in range(total_time_step//time_step_precision):
        ...
        model = AdaBoostClassifier(base_estimator=tree,learning_rate=train_hyperparameters.get('learning_rate'),
                    n_estimators=train_hyperparameters.get('n_estimators'),algorithm=train_hyperparameters.get('algorithm'))

...
def ensembled_train_model(train_dataset_path,train_hyperparameters):
    ...
    model = VotingClassifier(estimators=[('rfc', rfc), ('svc', svc)], voting='hard')
    model.fit(train_X,train_y)
    pickle.dump(model, open('E:\\group project\\vortex_framework\\data\\trained_models\\ensembled\\time_step_57.pkl', 'wb'))
...
def ensembled_predict_model(dataset_path, model_path,k_value):
    ...
    plt.savefig('E:\\group project\\vortex_framework\\data\\model_prediction\\ensembled_model\\time_step_'+str(time_step)+'_vortex_plot.png',bbox_inches='tight')

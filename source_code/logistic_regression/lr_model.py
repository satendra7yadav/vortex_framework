...
def lr_train_model(train_dataset_path,train_hyperparameters):
    ...
    model_dir = os.path.join(train_hyperparameters.get('model_dir'),'logistic_regression',test_case)
    ...
    for i in range(total_time_step//time_step_precision):
        ...
        model = LogisticRegression(random_state=100,penalty=train_hyperparameters.get('penalty'),C=train_hyperparameters.get('C'),
                                solver =train_hyperparameters.get('solver'))

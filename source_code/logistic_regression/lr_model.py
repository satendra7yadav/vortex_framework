...
def lr_train_model(train_dataset_path,train_hyperparameters):
    ...
    model = LogisticRegression(random_state=100,penalty=train_hyperparameters.get('penalty'),C=train_hyperparameters.get('C'),solver =train_hyperparameters.get('solver'))
    model.fit(train_X,train_y)
    pickle.dump(model, open('E:\\group project\\grp_project\\vortex_framework\\data\\trained_models\\logistic_regression\\time_step_57.pkl', 'wb'))
...
def lr_predict_model(dataset_path, model_path,k_value):
    ...
    plt.savefig('E:\\group project\\grp_project\\vortex_framework\\data\\model_prediction\\logistic_regression_model\\time_step_'+str(time_step)+'_vortex_plot.png',bbox_inches='tight')
    plt.tight_layout()
    plt.show()

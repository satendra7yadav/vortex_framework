...
def svm_train_model(train_dataset_path,train_hyperparameters):
    ...
    model = svm.SVC(kernel=train_hyperparameters.get('kernel'), C=train_hyperparameters.get('C'), gamma=train_hyperparameters.get('gamma'))
    model.fit(train_X,train_y)
    pickle.dump(model, open('E:\\group project\\grp_project\\vortex_framework\\data\\trained_models\\svm\\time_step_57.pkl', 'wb'))
...
def svm_predict_model(dataset_path, model_path,k_value):
    ...
    plt.savefig('E:\\group project\\grp_project\\vortex_framework\\data\\model_prediction\\svm_model\\time_step_'+str(time_step)+'_vortex_plot.png',bbox_inches='tight')
    plt.tight_layout()
    plt.show()
